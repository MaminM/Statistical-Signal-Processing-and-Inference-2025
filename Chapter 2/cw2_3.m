...
for p=1:MAX_MODEL_ORDER
    ...
    predictions_ = filter(-yw_coeffs, 1, ss_scaled);
    % the first p values will have to be the original data
    predictions_(1:length(yw_coeffs)) = ss_scaled(1:length(yw_coeffs));
    
    % Calculate residuals and MSE
    residuals = ss_scaled - predictions_;
    CMSE(1, p) = N * mean(residuals.^2);
    
    Ep = CMSE(1,p);
    MDL(1, p) = log10(Ep) + (p * log10(N)) / N;
    AIC(1, p) = log10(Ep) + (2*p) / N;
    AICc(1, p) = AIC(1,p) + (2 * p * (p + 1)) / (N - p - 1);

    % % save in predictions
    % predictions(p,1:length(predictions_)) = predictions_;
...
figure
hold on
% plot(1:MAX_MODEL_ORDER, MDL, 'DisplayName', 'MDL')
plot(1:MAX_MODEL_ORDER, CMSE, 'DisplayName', 'Cumulative Squared Error')
xlabel('model order p')
title('MDL for sunspot data')
